...
--- @field selection SelectionContext
--- @field original_lines string[]
--- @field tracker EditRangeTracker
...
--- @param application EditApplication
...
local function current_apply_range(application, old_start, old_end)
  local selection = application.selection
...
--- @param application EditApplication
local function close(application)
  if application.closed then
    return
  end

  application.closed = true
  clear_edit_range(application.source_bufnr, application.tracker)
end

--- Start tracking an Edit Selection application.
--- @param source_bufnr integer
--- @param selection SelectionContext
--- @return EditApplication
function M.start(source_bufnr, selection)
  return {
    source_bufnr = source_bufnr,
    selection = selection,
    original_lines = vim.api.nvim_buf_get_lines(source_bufnr, 0, -1, false),
    tracker = track_edit_range(source_bufnr, selection.start_line, selection.end_line),
    closed = false,
  }
end

--- Stop tracking without applying source changes.
--- @param application EditApplication
function M.discard(application)
  close(application)
end
...
--- @param application EditApplication
function M.apply_changed_source(application)
  local selection = application.selection
  if selection.path == "" or not vim.api.nvim_buf_is_valid(application.source_bufnr) then
    close(application)
    return
  end
  ...
  local ok, err = pcall(function()
    ...
    local hunks = changed_hunks(application.original_lines, edited_lines)
    local old_start, old_end, new_start, new_end =
      changed_range(application.original_lines, edited_lines)
    if not old_start or not old_end or not new_start or not new_end then
      return
    end

    local start_row, end_row = current_apply_range(application, old_start, old_end)
    ...
    vim.api.nvim_buf_call(application.source_bufnr, function()
