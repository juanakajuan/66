...
function M.run()
	...
	ui.capture_prompt(" 66 edit ", "66 edit", "Edit66", function(instruction)
		local stop_status = start_implementing_status(source_bufnr, selection.start_line, selection.end_line)
		opencode.run(opencode.command(prompts.edit(instruction, selection), opencode.edit_title(instruction)), function(result, text)
			stop_status()
